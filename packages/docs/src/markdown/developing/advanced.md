...
| `--q-markdown-link-external-icon` | CSS image used as the mask for the external-link icon. |
| `--q-markdown-link-external-icon-display` | Display mode for the external-link icon. Set to `none` to hide it. |
| `--q-markdown-link-external-icon-size` | Width and height of the external-link icon. |
| `--q-markdown-link-external-icon-gap` | Space between an external link's text and icon. |
...
External links use a dependency-free SVG mask rather than an icon font. The mask inherits the link's current color and can be replaced or hidden with the external-link CSS variables:

```scss
.docs-markdown {
  --q-markdown-link-external-icon-size: 1em;
  --q-markdown-link-external-icon-gap: 0.25em;
}

.docs-markdown--without-external-icons {
  --q-markdown-link-external-icon-display: none;
}
```
...
| `.q-markdown--link-external` | External links opened in a new browser context. |
